...
void Room::Init()
{
	packet_sender_.Init(SendPacket);
...
{
	ResRoomUserList = [&](uint32_t session_id) {
		...
		for (auto other_session_id : session_ids_)
		{
			...
			user_list.push_back(std::make_pair(other_session_id, GetUserId(other_session_id)));
		}
	...
	NtfNewRoomAdmin = [&]() {
		packet_sender_.NtfNewRoomAdmin(session_ids_, admin_id_, GetUserId(admin_id_)); };

	ResRoomAdmin = [&](uint32_t session_id) {
		packet_sender_.ResRoomAdmin(session_id, admin_id_, GetUserId(admin_id_)); };
	...
	NtfStartOmokView = [&](uint32_t black_session_id, std::string balck_user_id, uint32_t white_session_id, std::string white_user_id) {
